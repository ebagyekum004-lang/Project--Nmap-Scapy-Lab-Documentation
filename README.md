...
**What I learnt from the Nmap lab:**
...
- Purpose: Launch the Scapy interactive environment with root privileges.

- Result: Successfully entered the Scapy interactive shell, which provides a Python environment with all Scapy functions pre-loaded for packet crafting and analysis.



2.2 Sniffing Traffic

- Command: sniff()

- Purpose: Capture all traffic on the default network interface.

- Traffic Triggering: To generate network traffic for capture:
1. Opening browser and visiting `10.6.6.23`
2. Performing network pings to various targets
...
- Result: Successfully captured all network traffic including HTTP requests to the target web server and ICMP echo requests/replies. The captured packets were stored in the variable `paro2` for further analysis.




2.3 Filtering ICMP Traffic

- Command:
sniff(iface="br-internal", filter="icmp", count=5)


- Purpose: Capture only ICMP protocol traffic on the internal bridge network interface, limiting to exactly 5 packets.
...
- Result: Successfully captured exactly 5 ICMP packets as they were generated by pinging the target system. The packets were stored in variable `paro3`, with `paro3.summary()` providing an overview of all captured ICMP exchanges.



**What I learnt from the Scapy lab**

The Scapy tool forms the foundation of network forensics, protocol analysis, and security monitoring. The ability to capture, filter, and analyze packets in real-time is invaluable for:
